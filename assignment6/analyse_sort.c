/* analyse_sort.c:
   C  program to  perform  a  best case,  worst  case  and avergae  case
   analysis  of different  sorting  algorithms for  varying input  sizes
   controlled by 3 parameters: start, stop and step.
*/
...
/* allocate memory using malloc; if it fails exit with error code 2 this
   should  be avoided  in a  routine which  writes to  a file  otherwise
...
/* the characteristic O(n), O(n^2) and  O(nlog n) running times are only
   observablle if the size of the input is large enough */
...
int main (int argc, char **argv)
{
    ...
    input_t *best = (input_t *) xmalloc (sizeof (input_t) * N, 2);
    input_t *worst = (input_t *) xmalloc (sizeof (input_t) * N, 2);
    input_t *average = (input_t *) xmalloc (sizeof (input_t) * N, 2);
    ...
    for (i = 0, j = start; i < N; i++, j += step) {
        ...
        best[i].ptr = (int *) xmalloc (sizeof (int) * j, 2);
        /* for worst and average cases we allocate double the necessary
           ...
           For example: [3, 77,..., 9,  3, 77,..., 9] 
                         0   1 ...  N-1 N  N+1 ... 2N-1 */
        worst[i].ptr = (int *) xmalloc (2 * sizeof (int) * j, 2);
        average[i].ptr = (int *) xmalloc (2 * sizeof (int) * j, 2);
    ...
    for (i = 0; i < 5; i++) {
        ...
        for (j = 0; j < N; j++) {
            ...
            /* For worst and average cases,  we need to restore array to
               original  state so  the  next sorting  algorithm sees  it
